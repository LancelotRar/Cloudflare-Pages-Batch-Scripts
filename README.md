...
配置完整的 `dns` 时，脚本会在 Pages 项目完成最终重部署后，查询 Zone 中的全部 DNS 记录，并按 `content` 精确匹配。找到记录后会将其名称、类型、代理状态和 TTL 修改为声明配置；记录不存在时创建，重复内容的记录会收敛为一条。DNS 查询、创建或修改失败会使当前账号失败，但不会影响后续账号继续部署。
