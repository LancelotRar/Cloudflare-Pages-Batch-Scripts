...
配置完整的 `dns` 时，脚本会在 Pages 项目完成最终重部署后，查询 Zone 中的全部 DNS 记录，并按 `type + name` 精确匹配。记录不存在时创建，内容、代理状态或 TTL 不一致时修改，类型变化时删除同名旧类型后创建目标类型，重复记录会收敛为一条。DNS 查询、创建或修改失败会使当前账号失败，但不会影响后续账号继续部署。
