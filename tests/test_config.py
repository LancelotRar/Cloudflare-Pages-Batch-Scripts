...
from cf_pages_batch_scripts.models import Account, Config, DnsConfig, PagesConfig
...
class TestParsePagesConfig:
    ...
    def test_env_is_nested_in_pages(self):
        pc = _parse_pages_config({"project_name": "p", "env": [{"name": "K", "type": "plain_text", "value": "v"}]})
        assert pc.env[0].name == "K"


class TestParseDnsConfig:
    def test_parses_auto_ttl(self):
        dns = _parse_dns_config({
    ...
    def test_ttl_numeric_int(self):
        dns = _parse_dns_config({"ttl": 300})
        assert dns.ttl == 300

    def test_ttl_numeric_string(self):
        dns = _parse_dns_config({"ttl": "120"})
        assert dns.ttl == 120

    def test_ttl_invalid_raises(self):
        with pytest.raises(ValueError, match="ttl"):
            _parse_dns_config({"ttl": "abc"})
