"""Tests for cf_pages_batch_scripts.api"""

import httpx
import pytest
from pytest_httpx import HTTPXMock

from cf_pages_batch_scripts import api as api_module
...
def _no_retry_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    """重试测试不真实 sleep（2/4s 退避），保持测试套件快速。"""
...
class TestApiMethods:
    ...
    def test_list_domains(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            url=f"{API_BASE}/accounts/test_account/pages/projects/p/domains",
            json={"success": True, "result": [{"name": "example.com"}]},
        )
        assert client.list_domains("p") == [{"name": "example.com"}]

    def test_list_domains_failure(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            url=f"{API_BASE}/accounts/test_account/pages/projects/p/domains",
            json={"success": False},
        )
        assert client.list_domains("p") is None
    ...
    def test_list_dns_records(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            url=f"{API_BASE}/zones/zone/dns_records?page=1&per_page=100&content.exact=target.pages.dev",
            json={"success": True, "result": [{"id": "r1"}], "result_info": {"total_pages": 1}},
        )
        assert client.list_dns_records("zone", "target.pages.dev") == [{"id": "r1"}]

    def test_list_dns_records_preserves_api_error(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            url=f"{API_BASE}/zones/zone/dns_records?page=1&per_page=100&content.exact=target.pages.dev",
            status_code=403,
        ...
        assert client.list_dns_records("zone", "target.pages.dev") is None
        assert client.last_error == "Cloudflare API 403 [9109]: Invalid access token"

    def test_create_dns_record(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            method="POST",
            url=f"{API_BASE}/zones/zone/dns_records",
            json={"success": True},
        )
        assert client.create_dns_record("zone", {"type": "CNAME"}) is not None

    def test_update_dns_record(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            method="PUT",
            url=f"{API_BASE}/zones/zone/dns_records/r1",
            json={"success": True},
        )
        assert client.update_dns_record("zone", "r1", {"type": "CNAME"}) is not None

    def test_delete_dns_record(self, client: CfApiClient, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            method="DELETE",
            url=f"{API_BASE}/zones/zone/dns_records/r1",
            json={"success": True},
        )
        assert client.delete_dns_record("zone", "r1") is not None
